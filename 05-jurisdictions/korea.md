...
**Status: current as at 13 August 2026 (KST) for the high-performance threshold note below only. The remainder of this guide is current as at early July 2026 and was not re-verified in this revision. Korea's AI Framework Act and its Enforcement Decree are in force. A grace period on administrative fines runs through the first year of enforcement. Verify current status before relying on anything below.**
...
3. **High-performance (large-scale) AI.** Systems trained above a cumulative compute threshold of 10^26 FLOPs, roughly ten times the EU's GPAI presumption threshold, carry safety obligations: lifecycle risk identification, assessment and mitigation, user protection measures and reporting of implementation outcomes to MSIT. The threshold deliberately captures only frontier-scale global developers.
   ...
   ⚠ **Two open points on this category, both requiring counsel.** *[As at 13 August 2026 (KST).]* First, the compute threshold does not stand alone: it operates together with criteria publicly notified by the Minister of Science and ICT, and the method for calculating cumulative compute is set by notice. Available sources conflict on whether the notified criteria operate **cumulatively** with the 10^26 threshold or as an **alternative** trigger, and that difference decides whether a sub-threshold system can be captured. Second, the **regulated actor is unresolved**: whether the obligation attaches to the party that performed the training run, the party that makes the model available in Korea, or both, and how cumulative compute is attributed across fine-tuning. Primary text of the final Enforcement Decree was not obtained for this revision. Do not scope this category from the FLOPs number alone.
